...
def shorten_name(full_name_str):
    ...
    # Strip whitespace and split by spaces to isolate the first name
    ...
    # If there is a surname, grab the absolute first character of it
    if len(parts) > 1:
...
def database_load_pipeline():
    try:
        df_p = conn.read(worksheet="Participants_NB", ttl=0)
        if df_p is None:
    ...
    # --- NEW RIGOROUS UNIFIED LOOKUP MAPPING ENGINE ---
    team_to_player = {}
    if not df_p.empty:
        for _, row in df_p.iterrows():
            ...
            # Grab names from both possible columns safely
            p1 = shorten_name(row.get("Participant Name", ""))
            p2 = shorten_name(row.get("Participant Name 2", ""))
            p2 = p2.strip()
            
            # Combine the valid names for this row
            row_players = []
            ...
            # Append or initialize team mappings
    ...
    # Apply the combined player strings directly onto the team dataframe rows as a permanent column
    if not df_t.empty:
        df_t["Player"] = df_t["Team"].apply(lambda x: team_to_player.get(str(x).strip().upper(), str(x).strip()))
    else:
        df_t["Player"] = []
        ...
        if not st.session_state["admin_authenticated"]:
                ...
                if st.button("💾 Save Participant Grid Changes", type="primary"):
                    try:
                        with st.spinner("Synchronizing database registry..."):
                            ...
                            conn.update(worksheet="Participants_NB", data=edited_p_df)
                            st.cache_data.clear()
            ...
            with adm_t2:
                ...
                if st.button("Wipe & Clear Global Datastores", disabled=not safety_checkbox):
                    ...
                    try:
                        conn.update(worksheet="Participants_NB", data=blank_p)
                        st.cache_data.clear()
